...
export type FileEntityType = "user" | "meeting";
...
export class FileStorageService {
  ...
  async uploadFile(input: UploadFileInput): Promise<StoredFileUploadResult> {
    ...
    const uploadResult = await this.adapter.uploadFile({ ...input, byteSize });
  ...
  async getFileById(id: string): Promise<StoredFile> {
    const [record] = await this.db.select().from(file).where(eq(file.id, id));
  ...
  async getFileStream(key: string): Promise<Readable> {
    return this.adapter.getFileStream(key);
  }
  ...
  async getSignedDownloadUrl(
    ...
    return this.adapter.getSignedDownloadUrl(key, expiresInSeconds);
  }
  ...
  generateEntityRef(entityType: FileEntityType, entityId: string) {
